...
- `src/lib/email.ts` — Resend-backed digest emails of top job matches (`sendDigestEmail`), gated by the `email_digest` config and `RESEND_API_KEY`. `buildDigestHtml` renders a dark-header layout with summary chips, an "Apply now" block surfacing remote STRONG/GOOD-FIT roles (remote = the originating search's `remote_filter` is `remote`; job location text is deliberately not used), fit-tier card sections, a compact weak-fit list, per-role color-coded search pills, and a Top-10-this-week section sorted by fit then score. Testable via `/api/email/test` (add `?empty=true` for the quiet-day fallback).
...
- `jobs/evaluate/route.ts` — bearer-key fetch → Claude eval → upsert for a **whole results page (up to 25 jobs)** in one request. Jobs are processed **concurrently** with the same design as the scrape pipeline's `processJobBatch`: a bounded fetch pool (`mapPool`, `fetch_concurrency`) overlapped with a wider eval pool (`createSemaphore`, `eval_concurrency`), both imported from `@/lib/scraper/pipeline`. There is **no per-job delay** — LinkedIn load stays bounded by `fetch_concurrency` (5), matching the scheduled scraper. This replaced the old serial loop (3 jobs/request, 1500ms sleep between each) that made a 25-job page take minutes; it now takes ~20-30s. Guards: 100 evals/user/hour soft cap (429), a 90s deadline (`DEADLINE_MS`, under the 120s `maxDuration`) checked in the fetch worker before starting each job — jobs not started by then return retryable `error` results; upserts use `ignoreDuplicates` on `(user_id, job_url)` so a racing scheduled scrape's row always wins, and transient fetch/eval failures persist nothing (retried on the next run, never written as permanent skips). Rows it creates get `search_query: "Chrome Extension"` and a slug-free canonical job URL.

The client (`content.js`) sends the whole page in one request via `BATCH_SIZE = 25`, kept in sync with the route's schema cap; the server handles concurrency, so the client no longer loops over small serial batches.

Request schemas live in `src/lib/validation.ts` (`extensionLookupSchema` — 50 IDs; `extensionEvaluateSchema` — 25 jobs); the strict `^\d{8,}$` job-ID regex doubles as the injection guard for URLs built from IDs.
